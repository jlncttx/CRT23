...
YEARS=1947:2022 # version article
#YEARS=1947:2023 # update
...
# Output directory
ODIR=paste0(SCANDIR,"scans_1d/mf/")        # version article
#ODIR=paste0(SCANDIR,"scans_1d/mf_new/")    # update
system(paste("mkdir -p",ODIR))

#---------------------------------------------------------------------------------
# Get data: XLOC data.frame $date $var = daily values at LOC
#---------------------------------------------------------------------------------
# Local data for 1D-analysis (Meteo-France)
XLOC=myno("../data/mf/Thermal_index_FR_1947-2022.nc",varid=VAR) # version article
#XLOC=myno("../data/mf/Thermal_index_FR.nc",varid=VAR)           # update
XLOC=data.frame(date=mdy2yyyymmdd(time2mdy(XLOC$time,XLOC$timeu)),var=XLOC$var)
...
for (m in c("calend","locmax","locmin",
	    "annmax",#"annmax1","annmax2","annmax3",
	    "annmin")){#},"annmin1","annmin2","annmin3")){
  # Method to use
  METHOD=m
